        ...
        std::string message(int e) const{
            switch(e){
                case invalid_response:
                    return "Server response cannot parsed.";
                    break;
                default:
                    return "Unknow error.";
                    break;
...
private:
    net::streambuf& get_response_buf(){
...
class HTTPRequest{
    ...
    static const unsigned DEFAULT_PORT = 12;
    // Request parameters
    std::string m_host;
    unsigned m_port;
    std::string m_uri;
    ...
    // Callback to be called when req completes
    Callback m_callback;
    ...
    ///////////////////////////////////////////////////]
    HTTPRequest (net::io_context& ios, unsigned id):
        m_port(DEFAULT_PORT),
        m_id(id),
        m_callback(nullptr),
        m_sock(ios),
...
public:
    ...
    void execute(){
        ...
        // Ensure thar precorditions hold
        assert(m_port > 0);
        ...
        tcp::resolver::query resolver_query(m_host, std::to_string(m_port), tcp::resolver::query::numeric_service);
        
        std::unique_lock<std::mutex> cancel_lock(m_cancel_mux);

        if(m_was_cancelled){
            cancel_lock.unlock();
            on_finish(system::error_code(net::error::operation_aborted));
            return;
        }
...
private:
        ...
        tcp::resolver::iterator it)
    {
        ...
        net::async_connect(m_sock, it, [this](const system::error_code& ec, tcp::resolver::iterator it)
        {
            on_connection_established(ec, it);
        });
    }
    void on_connection_established(const system::error_code& ec, tcp::resolver::iterator it)
    {
        if(ec.value()){
            on_finish(ec);
            return;
        ...
        net::async_write(m_sock, net::buffer(m_request_buf),[this](const system::error_code& ec, std::size_t bytes_transferred)
        {
            on_request_sent(ec, bytes_transferred);
        });
    }
    void on_request_sent(const system::error_code& ec, std::size_t bytes_transferred)
    {
        if(ec.value())
        {
            on_finish(ec);
        ...
        m_sock.shutdown(tcp::socket::shutdown_send);
        ...
        net::async_read_until(m_sock, m_response.get_response_buf(), "\r\n",
                                [this](const system::error_code& ec, std::size_t bytes_transferred){
                                    on_status_line_received(ec, bytes_transferred);
                                });
    }
    void on_status_line_received(const system::error_code& ec, std::size_t bytes_transferred)
    {
        if(ec.value())
        {
            on_finish(ec);
        ...
        // At this point the status line is success
        // recieved and parsed.
    ...
    void on_headers_received(const system::error_code& ec, std::size_t bytes_transferred)
    {
        if(ec.value()){
            on_finish(ec);
            return;
        ...
        net::async_read(m_sock, m_response.get_response_buf(),
                        [this](const system::error_code& ec, std::size_t bytes_transferred)
                        {
    ...
    void on_response_body_received(const system::error_code& ec, std::size_t bytes_transferred)
    {
        if(ec == net::error::eof)
            on_finish(system::error_code());
        else
            on_finish(ec);
    }
...
const system::error_code& ec)
{
    if (!ec.value()) {
        ...
        << " has completed. Status code: "<< response.get_status_code() 
